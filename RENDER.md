...
3. Use the repository's `render.yaml` file. It defines one Docker web service named `launch-weather-replay`, a `0.5c-512mb` compute plan (0.5 CPU, 512 MB RAM), and a 10 GB persistent disk. No Redis, database, GitHub token, or KSC browser cookies are needed.
4. Review Render's price before confirming deployment. As checked September 11, 2026, the compute plan is $7/month and disk storage is $0.25/GB/month, so these resources total about $9.50/month, plus any workspace charges, usage overages and tax. This is not a billing cap. Verify the current total in Render. Do not select the free service: it cannot attach this persistent disk.
...
## Smaller-server trial

[Capacity trial on September 11, 2026](https://github.com/cyclonecizek/LaunchWeatherReplay/actions/runs/34650596808) passed using the production container with a hard 512,000,000-byte memory limit, no swap, and 0.5 CPU. The real June 25, 2024, 21:00–23:00 UTC case with a 60-minute lightning trail produced a 176,696,795-byte ZIP with 23 radar volumes, field mills, and MERLIN. The public server remained responsive, the downloaded ZIP passed all CRC checks, and there were zero out-of-memory events. The trial took 143 seconds on the test host; Render timing may differ.

Wind towers returned no plottable observations for the selected 54-foot height, so this was an explicitly marked partial package. That source gap remains unresolved. This validates one real two-hour workload on the smaller memory/CPU budget, not every possible 12-hour window or a tower-rich scenario.
...
- Source requests use bounded chunks. Individual source-response safety limits remain; large or unusually active windows may still need splitting. A two-hour build timeout and separate JavaScript heap limits (64 MB for the web server, 224 MB for the build worker) leave room for native buffers and ZIP creation within the smaller server. Larger or more active scenarios may still need splitting.
...
The production Docker image installs Python for ZIP creation, plus siphon 0.11.0 in `/opt/siphon` for the KXMR sounding lookup, and validates the KSC certificate chain at build time. The `Verify Render server` workflow builds the image, starts it with persistent storage, checks the public form/health endpoint, verifies the test ZIP, and probes KSC TLS. This automated verification runs with a 512 MB container limit, no swap, and half a CPU. A manual run also exercises the real June 25, 2024, 21:00–23:00 UTC scenario with a 60-minute lightning trail via the public form API, checks server responsiveness, verifies the downloaded ZIP, and checks for out-of-memory events. These checks are independent of visitor requests.
